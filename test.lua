-- [[ BAGIAN VALIDASI - JANGAN DIUBAH ]]
local HttpService = game:GetService("HttpService")
local HWID = game:GetService("RbxAnalyticsService"):GetClientId()

-- GANTI URL INI: Ambil dari menu Triggers di Cloudflare kamu
local URL_SERVER = "https://cek-lisensi.raeliosx.workers.dev" 

-- User harus isi ini sebelum execute
_G.ScriptKey = _G.ScriptKey or "MASUKKAN_KEY_DISINI"

local function Validasi()
    local sukses, hasil = pcall(function()
        return HttpService:PostAsync(URL_SERVER, HttpService:JSONEncode({
            key = _G.ScriptKey,
            hwid = HWID
    ...
    if sukses then
        local data = HttpService:JSONDecode(hasil)
    ...
    return false, "Server Error / URL Salah"
end

local boleh, pesan = Validasi()

if not boleh then
    -- Jika key salah atau expired, script berhenti total
    warn("🚫 [RAZUX AUTH]: " .. pesan)
    return 
end

print("✅ [RAZUX AUTH]: " .. pesan)

-- =====================================================
-- TEMPELKAN SELURUH ISI SCRIPT RAZUX.LUA ASLIMU DI BAWAH INI
-- =====================================================

-- Script asli kamu (Cleanup System, Variabel, WindUI, dll) dimulai dari sini...
