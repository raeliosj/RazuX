            ...
            if rawName ~= "" and rawName ~= "Item Name" then
                ...
                local data = {
                    ["embeds"] = {{
                        ...
                        ["fields"] = {
                            {["name"] = "💎 Rarity", ["value"] = "Tier " .. tier .. " (" .. (rarityNames[tier] or "Common") .. ")", ["inline"] = true},
...
local sectionWeb = TabPlayer:Section({ Title = "Inventory Visualizer", Opened = true})

sectionWeb:Button({
    Title = "Send Visual Inventory",
    Desc = "Kirim detail: Gambar, Weight, Value, Mutation",
    Icon = "image",
    Callback = function()
        SendFullInventoryToDiscord()
    end
})

sectionWeb:Button({
    Title = "Refresh Data",
    Desc = "Klik ini kalo tas belum kebaca",
    Icon = "refresh-cw",
    Callback = function()
        local pGui = game:GetService("Players").LocalPlayer:FindFirstChild("PlayerGui")
        if pGui and pGui:FindFirstChild("Inventory") then
            ...
            WindUI:Notify({ Title = "Success", Content = "Inventory Refreshed!", Duration = 2 })
        end
    end
})
