...
local function Reg(id, element)
    ForkyConfig:Register(id, element)
...
local BaseFolder = "WindUI/" .. (Window.Folder or "Forky") .. "/config/"
...
do
    ...
    _G.Forky_AutoAcceptTradeEnabled = false 
    ...
    if PromptController and PromptController.FirePrompt and Promise then
        ...
        PromptController.FirePrompt = function(self, promptText, ...)
            ...
            if _G.Forky_AutoAcceptTradeEnabled and type(promptText) == "string" and promptText:find("Accept") and promptText:find("from:") then
        ...
        warn("[Forky] Gagal memuat PromptController/Promise untuk Auto Accept Trade.")
    end
end
        ...
        if data then
            ...
            if targetPlayer.Character then
                for _, v in ipairs(targetPlayer.Character:GetChildren()) do
                    if v.Name == "ForkyESP" and v:IsA("BillboardGui") then pcall(function() v:Destroy() end) end
                end
            end
        end
    end
...
do
    ...
    task.spawn(function()
        ...
        if S1 and FishingController then
            ...
            FishingController.RequestChargeFishingRod = function(...)
                if _G.Forky_BlatantActive then return end 
                return Old_Charge(...)
            end
            FishingController.SendFishingRequestToServer = function(...)
                if _G.Forky_BlatantActive then return false, "Blocked by Forky" end
                return Old_Cast(...)
            end
        end
    end)
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if _G.Forky_BlatantActive and not checkcaller() then
    ...
    local function SuppressGameVisuals(active)
        ...
        if active then
            task.spawn(function()
                ...
                while _G.Forky_BlatantActive do
...
local function GetItemsToFavorite()
    ...
    local accept = trade:Toggle({
        ...
        Value = false,
        Callback = function(state)
            _G.Forky_AutoAcceptTradeEnabled = state
...
do
    ...
    local function CreateKaitunUI()
        local old = game.CoreGui:FindFirstChild("ForkyKaitunStats")
        if old then old:Destroy() end
        local sg = Instance.new("ScreenGui")
        sg.Name = "ForkyKaitunStats"
        ...
        txt("KAITUN Forky (PREMIUM)", 0.2, Color3.fromRGB(255,0,255), 35)
        ...
        Callback = function(state)
            ...
            if state then
                ...
                if not _G.BlackScreenGUI then
                    ...
                    _G.BlackScreenGUI.Name = "Forky_BlackBackground"
        ...
        Callback = function(state)
            ...
            if state then
                ...
                for _, gui in ipairs(PlayerGui:GetChildren()) do
                    if gui:IsA("ScreenGui") and gui.Name ~= "WindUI" and gui.Name ~= "CustomFloatingIcon_Forky" then
...
do
    ...
    local WEBHOOK_URL = ""
    local WEBHOOK_USERNAME = "Forky Notify" 
    ...
    local GLOBAL_WEBHOOK_USERNAME = "Forky | Community"
    ...
    local function onFishObtained(itemId, metadata, fullData)
        local success, results = pcall(function()
            ...
            if isWebhookEnabled and WEBHOOK_URL ~= "" and isUserFilterMatch then
                local title_private = string.format("<:TEXTURENOBG:1438662703722790992> Forky | Webhook\n\n<a:ChipiChapa:1438661193857503304> New Fish Caught! (%s)", fishName)
                ...
                local embed = {
                    ...
                    footer = {
                        text = string.format("Forky Webhook • Total Caught: %s • %s", caughtDisplay, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
            ...
            if isGlobalTarget and GLOBAL_WEBHOOK_URL ~= "" then 
                ...
                local title_global = string.format("<:TEXTURENOBG:1438662703722790992> Forky | Global Tracker\n\n<a:globe:1438758633151266818> GLOBAL CATCH! %s", fishName)
                ...
                local globalEmbed = {
                    ...
                    thumbnail = { url = imageUrl },
                    footer = {
                        text = string.format("Forky Community| Player: %s | %s", censoredPlayerName, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
        ...
        if not success then
            warn("[Forky Webhook] Error processing fish data:", results)
        end
    end
...
do
    ...
    local SelectedConfigName = "Forky" -- Default
    local BaseFolder = "WindUI/" .. (Window.Folder or "Forky") .. "/config/"
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            ...
            -- 1. Save ke config utama dulu ("Forky.json")
            ForkyConfig:Save()
            task.wait(0.1)

            -- 2. Jika nama beda, salin isi "Forky.json" ke "NamaBaru.json"
            if SelectedConfigName ~= "Forky" then
                local success, err = pcall(function()
                    local mainContent = readfile(BaseFolder .. "Forky.json")
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            if SelectedConfigName == "" or SelectedConfigName == "Forky" then 
            ...
            if isfile(path) then
                ...
                RefreshConfigList(ConfigDropdown)
                ConfigNameInput:Set("Forky")
                SelectedConfigName = "Forky"
            else
    ...
    --ConfigSection:Paragraph({
       ...
       -- Desc = "Script otomatis menyimpan ke 'Forky.json' setiap 30 detik.",
      --  Icon = "info"
 --   })
end
...
do
    ...
    about:Section({
        Title = "Join Discord Server Forky",
        TextSize = 20,
    })

    about:Paragraph({
        Title = "Forky Community",
        ...
        Buttons = {
            {
                ...
                Callback = function()
                    ...
                    WindUI:Notify({
                        Title = "Link Disalin!",
                        Content = "Link Discord Forky berhasil disalin.",
                        Duration = 3,
                        Icon = "copy",
                    })
                end,
...
local function CreateFloatingIcon()
    local existingGui = PlayerGui:FindFirstChild("CustomFloatingIcon_Forky")
    if existingGui then existingGui:Destroy() end

    local FloatingIconGui = Instance.new("ScreenGui")
    FloatingIconGui.Name = "CustomFloatingIcon_Forky"
...
WindUI:Notify({ Title = "Forky Was Loaded", Content = "Press [F] to open/close the menu", Duration = 5, Icon = "info" })
-- [[ AUTO LOAD & SAVE LOOP ]]
task.spawn(function()
    task.wait(2) -- Tunggu UI load sempurna
    
    -- Ganti Load biasa dengan Smart Load
    -- Default load "Forky", atau nama config terakhir user kalau kamu simpan
    SmartLoadConfig("Forky") 
    
    -- Auto Save Loop (Setiap 30 detik) -- Save tetap pakai cara biasa gapapa
    while true do
         task.wait(10)
         pcall(function() ForkyConfig:Save() end)
    end
end)
InitializeIcon()
