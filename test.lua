...
if not isfolder("Lynx") then
    makefolder("Lynx")
end
if not isfolder("Lynx/Config") then
    makefolder("Lynx/Config")
end
...
-- Force save (always saves, ignores AutoSaveEnabled)
...
-- Auto-save (respects AutoSaveEnabled flag)
function SaveConfig()
    -- Check if auto-save is disabled
    if _G.AutoSaveEnabled == false then
        return
    end
    ForceSaveConfig()
end
...
function ResetToDefaults()
    -- Clear ConfigData (keep version)
    local version = ConfigData._version
    ConfigData = { _version = version }
    
    -- Reset all UI elements to their defaults
    for key, element in pairs(Elements) do
...
local Lynx = {}
function Lynx:MakeNotify(NotifyConfig)
    local NotifyConfig = NotifyConfig or {}
    NotifyConfig.Title = NotifyConfig.Title or "Lynx"
    NotifyConfig.Description = NotifyConfig.Description or "Notification"
    NotifyConfig.Content = NotifyConfig.Content or "Content"
    NotifyConfig.Color = NotifyConfig.Color or Color3.fromRGB(255, 140, 0)
...
function Lynx:Window(GuiConfig)
    GuiConfig              = GuiConfig or {}
    GuiConfig.Title        = GuiConfig.Title or "Lynx"
    GuiConfig.Footer       = GuiConfig.Footer or "Lynx >:D"
    GuiConfig.Color        = GuiConfig.Color or Color3.fromRGB(255, 140, 0)
    GuiConfig["Tab Width"] = GuiConfig["Tab Width"] or 120
    GuiConfig.Version      = GuiConfig.Version or 1
    GuiConfig.Image        = GuiConfig.Image or "104332967321169"  -- Default Lynx icon
    ...
    LynxGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    LynxGui.Name = "LynxGui"
    LynxGui.ResetOnSpawn = false
    LynxGui.Parent = game:GetService("CoreGui")
    ...
    DropShadowHolder.ZIndex = 0
    DropShadowHolder.Name = "DropShadowHolder"
    DropShadowHolder.Parent = LynxGui

    DropShadowHolder.Position = UDim2.new(0, (LynxGui.AbsoluteSize.X // 2 - DropShadowHolder.Size.X.Offset // 2), 0,
        (LynxGui.AbsoluteSize.Y // 2 - DropShadowHolder.Size.Y.Offset // 2))
    ...
    function GuiFunc:DestroyGui()
        if CoreGui:FindFirstChild("LynxGui") then
            LynxGui:Destroy()
        end
    end
    ...
    function Tabs:AddTab(TabConfig)
        ...
        function Sections:AddSection(Title, AlwaysOpen)
            ...
            SectionDecideFrame.Visible = false -- Initial state
            ...
            SectionAdd.Visible = false -- Initial state
            ...
            function Items:AddToggle(ToggleConfig)
                ...
                -- Store original default BEFORE config overwrite
                local originalDefault = ToggleConfig.Default

                local configKey = "Toggle_" .. ToggleConfig.Title
                if ToggleConfig.Save ~= false and ConfigData[configKey] ~= nil then
                    ToggleConfig.Default = ConfigData[configKey]
                end

                local ToggleFunc = { Value = ToggleConfig.Default, Default = originalDefault }
            ...
            function Items:AddSlider(SliderConfig)
                ...
                -- Store original default BEFORE config overwrite
                local originalDefault = SliderConfig.Default
                ...
                local SliderFunc = { Value = SliderConfig.Default, Default = originalDefault }
